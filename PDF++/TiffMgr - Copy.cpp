...
void CTiffMgr::EncodeG4Block( const CSharedBuff& buffSrc, CSharedBuff& buffDest, int iBPP, int iWidth, int iHeight, int iComp  )
{
	...
	if( buffSrc.GetLength()==0 )
		throw std::exception( "EncodeG4Block: No source data provided." );
	...
	try
	{
		...
		byte* pOut = &buffDest.get()[0];   // pointer to the first element in arr
		...
		TIFF* fpTiff = TIFFClientOpen ("dummy", "w", (thandle_t)0,
	...
	#ifdef _DEBUG
			size_t dwCompressedSize = buffDest.GetLength();//(g_hMem != 0 ? ::GlobalSize( g_hMem ) : 0);
...
void CTiffMgr::DecodeG4Block( const CSharedBuff& buffSrc, CSharedBuff& buffDest, int iWidth, int iHeight, int BPP, int K )
{
	...
	if( buffSrc.GetLength()==0 )
		throw std::exception( "DecodeG4Block: No source data provided." );
...
/*
			...
			{
				d = ::GlobalSize(h);
				HANDLE hFile = CreateFileA("C:\\Projects\\Testfiles\\ContentsOut.tif",
					GENERIC_WRITE,
				...
				{
					BOOL bb= ::WriteFile( hFile, ::GlobalLock(h), d, &d, NULL );::GlobalUnlock(h);
					CloseHandle(hFile);
				}
			}*/
			...
			/*if( g_pTemp )
			{
				LARGE_INTEGER li = {NULL};
				li.LowPart = 0;
				g_pTemp->Seek( li, STREAM_SEEK_SET, NULL);


/////////////////////////////////////////////
				//IStream* pStm = NULL;
				fpTiff = TIFFClientOpen ("dummy", "r", (thandle_t)&pStm,
					&procs.CCITT_ReadProc2, &procs.CCITT_WriteProc, &procs.CCITT_SeekProc2,
					&procs.CCITT_CloseProc, &procs.CCITT_SizeProc2, &procs.CCITT_MapFileProc, &procs.CCITT_UnmapFileProc);
				if( fpTiff!=NULL )
				{

					dw  =size_tAlignedBytes(iWidth,BPP)*iHeight;
					HGLOBAL hUcompressedBits = NULL;
					if((hUcompressedBits = ::GlobalAlloc(GPTR, dw) ) != NULL)
						if( SUCCEEDED(::CreateStreamOnHGlobal( hUcompressedBits, true, &pStm )))
							dw = TIFFReadEncodedStrip( fpTiff, 0, (tdata_t)::GlobalLock(hUcompressedBits), dw); ::GlobalUnlock(hUcompressedBits);
				...
				g_pTemp = NULL;
//if( pStm )
					//::MessageBox(NULL, _T("DSDSDSDOIHE(W(UDSJDH"), _T(""), MB_OK );
				return pStm;
			...
			IStream* p = g_pTemp;
			g_pTemp = NULL;
			return p;////g_pTemp->Release();
			*/
		}
	}
	catch(...)
	{
		OutputDebugString(_T("TiffDecompress ERROR!!!!!!!!!!!!\n"));
	}
...
void CTiffMgr::TiffFromG4Block( const CSharedBuff& buffSrc, CSharedBuff& buffDest, int iWidth, int iHeight, int BPP, int K )
{
	...
	if( buffSrc.GetLength()==0 )
		throw std::exception( "TiffFromG4Block: No source data provided." );
	...
	{
		OutputDebugString(_T("TiffFromG4Block ERROR!!!!!!!!!!!!\n"));
	}
}
