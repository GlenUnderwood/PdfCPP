...
////////////////////
pdfGState::DashPattern::DashPattern( const DashPattern& rhs ) : m_Phase( 0 )
{
	operator=( rhs );
}

pdfGState::DashPattern& pdfGState::DashPattern::operator=( const DashPattern& rhs )
{
	m_Array.insert( m_Array.end(), rhs.m_Array.begin(), rhs.m_Array.end() );
	m_Phase = rhs.m_Phase;
	return *this;
}
bool pdfGState::DashPattern::operator==( const DashPattern& rhs )
{
